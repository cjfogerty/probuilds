...
| `ARCHITECTURE.md` | **Start here for the market-intelligence layer** — what the four layers do, what was found, what to build next |
| `METRICS.md` | **The metric contract** — every formula, denominator, evidence tier and blanking rule. A number not defined here does not get published |
| `scripts/` | Working, generalized Python. No dependencies beyond the stdlib |
| `reference/` | Federal geography and demand: ACS 2023 population/income by ZCTA, county and CBSA; OMB CBSA delineation; ZIP→ZCTA→county→CBSA crosswalks; ZCTA centroids |
| `dashboard/` | The four viewing surfaces. `markets.html` (Market Command) is the market-grain one |
| `data/` | Seat-model tables from the swim build, plus the full swim universe as a worked example |

## The market layer

The site census answers *who is where*. The market layer answers *how contested,
how served, and where the gap is* — which needs a denominator the site data does not
carry. `scripts/build_markets.py` joins every site to a CBSA and to ACS demand,
guards against implausible upstream values, and rolls up to market × category:

```bash
python3 scripts/build_markets.py     # ~2s, stdlib only
                                     # writes dashboard/data/markets.js + sites_geo.js
                                     # and build_report.json (read this before trusting a run)
```

Two warnings that came out of building it, both live in the current data:

- **Capacity had impossible values.** Eleven sites carried derived capacity in the
  millions or billions — one read 166 billion seats — which made national capacity
  and every fill-rate figure meaningless. All are `observed_max` on Jackrabbit.
- **Two sites held 12% of national enrollment.** 15,822 and 7,197 students at single
  locations against a median of 270, both `observed_max` on iClassPro, almost
  certainly class-occurrence double counting. With four mislabeled monthly prices,
  removing all of them moves estimated national annual tuition from $375M to $307M.

Both are flagged rather than deleted, and shown on the dashboard's Data quality tab.
Fixing them upstream is worth more than any new brand you could add.
